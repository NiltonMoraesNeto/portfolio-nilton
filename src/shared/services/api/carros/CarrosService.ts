...
const getAll = async (page = 1, filter = ''): Promise<TCarrosComTotalCount | Error> => {
    try {
        ...
        const urlRelativa = `/carros?_page=${page}&_limit=${Environment.LIMITE_DE_LINHAS}&montadora_like=${filter}`;
        ...
        console.error(error);
        return new Error((error as {message:string}).message || "Erro ao listar os dados.");
    }
};
